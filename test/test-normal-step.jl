...
@testset "Example 1" begin
    ...
    meta_dci = DCI.MetaDCI(nlp.meta.x0, nlp.meta.y0)
    ...
    z, cz, ncz, Jz, status = DCI.feasibility_step(nlp, x, cx, norm(cx), Jx, ρ, ctol, meta_dci;
    ...
    cxϵ = cons(nlp, xϵ)
    Jxϵ = jac(nlp, xϵ)
    zϵ, czϵ, ncz, Jz, statusϵ = DCI.feasibility_step(nlp, xϵ, cxϵ, norm(cxϵ), Jxϵ, ρ, ctol, meta_dci;
                                η₁ = 1e-3, η₂ = 0.66, σ₁ = 0.25, σ₂ = 2.0,
                                max_eval = 1_000, max_time = 60.,
                                )
...
@testset "Example 2, Mission: Impossible " begin
    ...
    cx = cons(nlp, xϵ)
    Jx = jac(nlp, xϵ)
    z, cz, ncz, Jz, status = DCI.feasibility_step(nlp, xϵ, cx, norm(cx), Jx, ρ, ctol, meta_dci;
                                η₁ = 1e-3, η₂ = 0.66, σ₁ = 0.25, σ₂ = 2.0,
                                max_eval = 1_000, max_time = 60.,
                                )
    @test status == :success
